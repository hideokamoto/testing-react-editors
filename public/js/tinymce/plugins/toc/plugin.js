...
var toc = (function () {
  ...
  var $_fd0pixsbjnlf45w6 = { create: create };

  var tocId = $_fd0pixsbjnlf45w6.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_81p7iksajnlf45w5.getTocClass(editor);
    var headerTag = $_81p7iksajnlf45w5.getTocHeader(editor);
    var selector = generateSelector($_81p7iksajnlf45w5.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_81p7iksajnlf45w5.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_81p7iksajnlf45w5.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_2pi2c4s6jnlf45w1.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_2pi2c4s6jnlf45w1.updateToc(editor);
    });
  };
  var $_ga6fjqs5jnlf45w0 = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_81p7iksajnlf45w5.getTocClass(editor);
  ...
  var $_dpv0uyscjnlf45w7 = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_2pi2c4s6jnlf45w1.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_81p7iksajnlf45w5.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_5mtsg3sdjnlf45wa = { register: register$1 };

  global.add('toc', function (editor) {
    $_ga6fjqs5jnlf45w0.register(editor);
    $_5mtsg3sdjnlf45wa.register(editor);
    $_dpv0uyscjnlf45w7.setup(editor);
  });
  function Plugin () {
  }
